...
exports.findLowAttendees = exports.findBranchAbsentees = exports.findAbsentStudents = exports.addPresentAttendance = exports.deleteStudent = exports.updateStudentInfo = exports.updateBranchOrBatch = exports.getStudent = exports.addStudent = void 0;
const batch_DAL_1 = require("../batch/batch.DAL");
const student_DAL_1 = require("./student.DAL");
async function addStudent(req, res, next) {
    try {
        const studentObject = req.body;
        if (!studentObject.firstName ||
        ...
        const batch = await (0, batch_DAL_1.findBatch)(studentObject.batch, studentObject.branch);
        if (!batch) {
        ...
        const student = await (0, student_DAL_1.addStudentInDB)(studentObject);
        await batch.save();
...
async function getStudent(req, res, next) {
    ...
    try {
        const student = await (0, student_DAL_1.getStudentById)(studentId);
        if (!student) {
...
async function updateBranchOrBatch(req, res, next) {
    try {
        ...
        const student = await (0, student_DAL_1.getStudentById)(id);
        console.log(student);
        ...
        const newBatch = await (0, batch_DAL_1.findBatch)(batch, branch);
        if (!newBatch) {
        ...
        await newBatch.save();
        await student.save();
        const previousBatch = await (0, batch_DAL_1.findBatch)(oldBatch, oldBranch);
...
exports.updateBranchOrBatch = updateBranchOrBatch;
...
async function deleteStudent(req, res, next) {
    const studentId = req.params.id;
    try {
        const student = await (0, student_DAL_1.deleteStudentById)(studentId);
        if (!student) {
            return next(res.status(404).send('No student found'));
        }
        const batch = await (0, batch_DAL_1.findBatch)(student.batch, student.branch);
        batch.branches.forEach((branch) => {
...
async function addPresentAttendance(req, res, next) {
    const studentId = req.params.id;
...
exports.addPresentAttendance = addPresentAttendance;
async function findAbsentStudents(req, res, next) {
    const date = req.params.date;
    try {
        const absentStudents = await (0, student_DAL_1.findAllAbsentStudents)(new Date(date).toString());
...
exports.findAbsentStudents = findAbsentStudents;
async function findBranchAbsentees(req, res, next) {
    const date = req.query.date;
    const batch = req.query.batch;
    const branch = req.query.branch;
    const currentSemester = req.query.currentSemester;
    try {
        const absentStudents = await (0, student_DAL_1.findBranchAbsentStudents)(batch, branch, currentSemester, new Date(date).toString());
        return res.status(200).send({ data: absentStudents });
    }
    catch (err) {
        return next(err);
...
exports.findBranchAbsentees = findBranchAbsentees;
async function findLowAttendees(req, res, next) {
    const value = parseInt(req.query.value);
    const batch = req.query.batch;
    const branch = req.query.branch;
    const currentSemester = req.query.currentSemester;
    try {
        const absentStudents = await (0, student_DAL_1.findLowattendenceStudents)(batch, branch, currentSemester, value);
        return res.status(200).send({ data: absentStudents });
    }
    catch (err) {
        return next(err);
...
exports.findLowAttendees = findLowAttendees;
